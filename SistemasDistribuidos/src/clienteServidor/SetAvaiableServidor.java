...
public class SetAvaiableServidor {
	...
	public void setAvaiable(JsonObject jsonRequest, PrintWriter out) throws IOException{
		...
		try {
			...
			if(!listOfRecruiter.stream().anyMatch(recruiter -> recruiter.getId() == recruterId)) {
				JsonObject jsonResponse = CreateJson.createResponse("SET_JOB_AVAILABLE","INVALID_TOKEN","");
			...
			String avaiable = (String) data.get("available");
			
			if(avaiable.isEmpty() || id.isEmpty()) {
				JsonObject jsonResponse = CreateJson.createResponse("SET_JOB_AVAILABLE", "INVALID_FIELD", "");
			...
			for(JobCandidate jobCandidate : listOfJobCandidates) {
				if(jobCandidate.getPersonId() == recruterId) {					
					...
					if(stringIdJob.equals(id)) {
						...
						if(data.containsKey("available")) {
							String changedAvaiable = (String) data.get("available");
							...
							if(!(changedAvaiable.equals("YES") || changedAvaiable.equals("NO"))) {
								...
								jobCandidate.setAvailable(avaiable);
								changes = true;
				...
				if(changes == true){
					...
					JsonObject jsonResponse = CreateJson.createResponse("SET_JOB_AVAILABLE", "SUCCESS", "");
					jsonResponse.remove("data");
					jsonResponse.put("data", dataResponse);
					System.out.println("Enviado para o cliente: " + jsonResponse);
					out.println(CreateJson.toJsonString(jsonResponse));		
				}else {
					JsonObject jsonResponse = CreateJson.createResponse("SET_JOB_AVAILABLE", "SKILL_NOT_EXIST", "");
					System.out.println("Mandando para o cliente: " + jsonResponse);
					out.println(CreateJson.toJsonString(jsonResponse));
					return;
				}
